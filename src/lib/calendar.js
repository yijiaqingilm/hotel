...
var Calendar = function ($, app, params) {
  var p = this
  var defaults = {
  ...
  // 查询date在指定目标中的信息，不存在返回null
  function queryDate (targetArr, date) {
    let low = 0
    ...
    while (low <= high) {
      let mind = Math.floor((low + high) / 2)
      let mindDateInfo = targetArr[mind]
      let mindDate = string2Date(mindDateInfo.date)
      if (mindDate.getTime() > date) {
        high = mind - 1
      }
      if (mindDate.getTime() < date) {
        low = mind + 1
  ...
  p.updateValue = function (onlyHeader) {
    ...
    if (p.container && p.container.length > 0) {
      let selected = p.wrapper.find('.picker-calendar-day-selected')
      selected.forEach((item) => {
        let year = $(item).data('year')
        let month = $(item).data('month')
        let day = $(item).data('day')
        $(item).find('.priceTxt').html(sessionStorage.getItem(`date_${year}_${month}_${day}`))
      })
      selected.removeClass('picker-calendar-day-selected')
      p.wrapper.find('.picker-calendar-day-selectedPass').removeClass('picker-calendar-day-selectedPass')
      var valueDate
      if (p.params.rangePicker && p.value.length === 2) {
        let startYear = p.value[0].getFullYear()
        let startMonth = p.value[0].getMonth()
        let startNumber = p.value[0].getDate()

        let endYear = p.value[1].getFullYear()
        let endMonth = p.value[1].getMonth()
        let endNumber = p.value[1].getDate()
        let startDate = p.wrapper.find('.picker-calendar-day[data-date="' + startYear + '-' + startMonth + '-' + startNumber + '"]')
        let endDate = p.wrapper.find('.picker-calendar-day[data-date="' + endYear + '-' + endMonth + '-' + endNumber + '"]')
        for (i = new Date(p.value[0]).getTime() + 24 * 60 * 60 * 1000; i < new Date(p.value[1]).getTime(); i += 24 * 60 * 60 * 1000) {
          valueDate = new Date(i)
          let valueDateInfo = queryDate(this.params.roomCalendar, i)
          if (valueDateInfo && !valueDateInfo.issell) {
            app.alert('您选择的日期中有不可用日期', '友情提示')
            return
          }
          p.wrapper.find('.picker-calendar-day[data-date="' + valueDate.getFullYear() + '-' + valueDate.getMonth() + '-' + valueDate.getDate() + '"]').addClass('picker-calendar-day-selectedPass')
        }
        startDate.addClass('picker-calendar-day-selected')
        endDate.addClass('picker-calendar-day-selected')
        sessionStorage.setItem(`date_${startYear}_${startMonth}_${startNumber}`, startDate.find('.priceTxt').html())
        sessionStorage.setItem(`date_${endYear}_${endMonth}_${endNumber}`, endDate.find('.priceTxt').html())
        startDate.find('.priceTxt').html('入住')
        endDate.find('.priceTxt').html('离店')
      }
      else {
        for (i = 0; i < p.value.length; i++) {
          ...
          let startDate = p.wrapper.find('.picker-calendar-day[data-date="' + year + '-' + month + '-' + day + '"]')
          startDate.addClass('picker-calendar-day-selected')
          sessionStorage.setItem(`date_${year}_${month}_${day}`, startDate.find('.priceTxt').html())
          startDate.find('.priceTxt').html('入住')
        }
      }
    }
  ...
  p.initCalendarEvents = function () {
    ...
    var isTouched, isMoved, touchStartX, touchStartY, touchCurrentX, touchCurrentY, touchStartTime, touchEndTime,
      startTranslate, currentTranslate, wrapperWidth, wrapperHeight, percentage, touchesDiff, isScrolling
    ...
    function handleDayClick (e) {
      ...
      let date = new Date(dateYear, dateMonth, dateDay, 0, 0, 0)
      // 增加租赁判断
      if (day.hasClass('picker-calendar-day-selected')) {
        return
      }
      if (day.hasClass('picker-calendar-day-lease')) {
        if (p.value && p.value.length === 1 && (date.getTime() - p.value[0].getTime()) === 24 * 60 * 60 * 1000) {
        } else {
          return
        }
  ...
  p.monthHTML = function (date, offset) {
    ...
    for (i = 1; i <= rows; i++) {
      ...
      for (j = 1; j <= cols; j++) {
        ...
        // 增加disabled 判断
        let {roomCalendar} = p.params
        let dayDateInfo = null
        if (roomCalendar && roomCalendar.length > 0) {
          dayDateInfo = queryDate(roomCalendar, dayDate)
        }
        dayDate = new Date(dayDate)
        var dayYear = dayDate.getFullYear()
        var dayMonth = dayDate.getMonth()
        let dateTxt = dayDate.getDate()
        let priceTxt = ''
        let isHoliday = false
        let holidayName = ''
        let isSell = false
        if (dayDateInfo) {
          isHoliday = dayDateInfo.isHoliday
          holidayName = dayDateInfo.holidayName
          priceTxt = '$' + dayDateInfo.price
          isSell = dayDateInfo.issell
          if (isHoliday) {
            dateTxt = holidayName
          }
          if (!dayDateInfo.issell) {
            priceTxt = '已租'
            addClass += ' picker-calendar-day-lease'
          }
        }
        if (disabled) {
          addClass += ' picker-calendar-day-disabled'
        }
        rowHTML += `<div data-year="${dayYear}" data-month="${dayMonth}" data-day="${dayNumber}" data-issell="${isSell}" class="picker-calendar-day ${addClass}" data-date="${dayYear}-${dayMonth}-${dayNumber}">
                      <div>
                        <em>${dateTxt}</em>
                        <div class='priceTxt'>${priceTxt}</div>
                        <i class="rest"></i>
                      </div>
                    </div>`
      }
